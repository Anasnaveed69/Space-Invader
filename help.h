#pragma once
#include <windows.h>
#pragma comment(lib, "Winmm.lib") //it's telling the linker to include the Winmm.lib library in the build.

// Frequency in hertz (1500 Hz in this example)
int frequency = 1500;

// Duration in milliseconds (200 milliseconds in this example)
int duration = 200;
...
bool isKeyPressed(int& whichKey)   //whichKey passed as reference.... 
{
	...
	key = GetAsyncKeyState(80);  // 80 is the virtual key code for the 'P' key
	if (key == 1) {
...
void myPointingTriangle(int baseX, int baseY, int height, int width, int R, int G, int B, int fillR = 0, int fillG = 0, int fillB = 0)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
	...
	// Calculate the coordinates for the right-facing triangle
	int x1 = baseX - width / 2;
	int y1 = baseY - height / 2;
	int x2 = x1;
	int y2 = baseY + height / 2;
	int x3 = baseX + width / 2;
	int y3 = baseY;
